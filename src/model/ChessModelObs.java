...
public class ChessModelObs extends Observable implements ChessGameModel {
	...
	public ActionType move (int xInit, int yInit, int xFinal, int yFinal){
		...
		this.notifyObservers(new MessageModelToGUI(couleur, 
				xInit, yInit, xFinal, yFinal, ret)); 
		return ret;	
	}
	 ...
	 * 
	 *  * Cette m̩thode ne pourra pas ̻tre invoqu̩e par d'autres 
	 * client que la classe BoardGame elle m̻me
	 */
	@Override
	...
	public boolean pawnPromotion(int x, int y, String promotionType) {
		...
		if (ret) {
			this.notifyObservers(new MessageModelToGUI(getPieceColor(x, y),-1, -1, x, y, ActionType.PROMOTION));  
		}
		return ret;
	}
